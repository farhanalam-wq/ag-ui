...
export function ProductGrid({ products = [], title, className = "" }: ProductGridProps) {
  ...
  const featuredIndex = products.findIndex((p) => p.featured);
  const featuredProduct = featuredIndex !== -1 ? products[featuredIndex] : products[0];
  const siblingProducts = products.filter((_, idx) => (featuredIndex !== -1 ? idx !== featuredIndex : idx !== 0));
      ...
      {title && (
        <div className="flex items-center justify-between pb-1 border-b border-zinc-200 dark:border-zinc-800/80">
          ...
          <span className="text-[11px] font-mono text-zinc-400">
            {products.length} Products & Capabilities
          </span>
        </div>
      )}
        ...
        <div className="relative overflow-hidden rounded-2xl border border-zinc-300 dark:border-zinc-700/80 bg-gradient-to-br from-white via-zinc-50/50 to-zinc-100/30 dark:from-zinc-900/90 dark:via-zinc-900/40 dark:to-zinc-950/80 p-5 sm:p-6 shadow-xl backdrop-blur-md transition-all hover:border-brand-primary/50">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div className="space-y-2.5 flex-1 min-w-0">
              ...
              {featuredProduct.tags && featuredProduct.tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 pt-1.5">
                  {featuredProduct.tags.map((tag, idx) => (
                    <span
                      key={idx}
            ...
            <div className="hidden sm:flex items-center justify-center size-14 rounded-2xl bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-700/60 shadow-inner shrink-0 mt-1">
              <TechIcon name={featuredProduct.icon || featuredProduct.title} size={30} />
            </div>
          </div>
        </div>
      )}
      ...
      {siblingProducts.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
          {siblingProducts.map((prod, idx) => (
            <div
              key={idx}
              className="flex flex-col justify-between p-4 rounded-xl border border-zinc-200 dark:border-zinc-800/80 bg-white/60 dark:bg-zinc-900/40 hover:bg-zinc-50 dark:hover:bg-zinc-900/70 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-200 shadow-sm space-y-3"
            >
              <div className="space-y-2">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="size-8 rounded-lg bg-zinc-100 dark:bg-zinc-800/70 border border-zinc-200/80 dark:border-zinc-700/50 flex items-center justify-center shrink-0">
                    <TechIcon name={prod.icon || prod.title} size={18} />
                  </div>
                </div>
